...
ll fordfulkerson(vector<ll>* edges,ll n,map<pair<ll,ll>,ll>& um,ll e,ll x,ll dest){
	ll sum = 0;
	ll* parent = new ll[n];
...
void dfs(ll i,ll ind,vector<ll>* edges,ll n,map<pair<ll,ll>,ll>& mp,vector<ll>& v){
...
{ 
	...
	REP(i,1,n+1){
		vector<ll> v;
		dfs(i,i,edges,n,um,v);
		REP(j,0,v.size()){
			edges[i].push_back(n+v[j]);	
		} 
	}
	// REP(i,0,2*n+2){
	// 	cout<<i<<":";
	...
	cout<<n - fordfulkerson(edges,2*n + 2,um,e,source,dest)<<endl;
