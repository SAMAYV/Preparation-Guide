...
atomic<int> total_points_in_circle = 0;
...
auto parse_arguments(int argc, char** argv) {
	...
	if(argc != 3){
		cout << "Requires three arguments - num_threads, total_points and points_per_job\n";
		exit(-1); 
	}
	auto num_threads = stoi(argv[0]);
	auto total_points = stol(argv[1]);
	auto points_per_job = stol(argv[2]);

	return make_tuple(num_threads, total_points, points_per_job);
}
...
void calculate(int num_threads, long long total_points, long long points_per_job) {
  	...
  	for(long long i = 0; i < total_points; i += points_per_job) {
    	auto num_points = min(points_per_job, total_points - i);
...
int main(int argc, char** argv) 
{
    ...
    cout << (4.0 * total_points_in_circle) / total_points << '\n';
    return 0;
}
